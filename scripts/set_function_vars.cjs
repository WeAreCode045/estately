...
const KEY = env.APPWRITE_KEY;
if (!KEY) { console.error('No APPWRITE_KEY in .env'); process.exit(2); }

const vars = [
  { key: 'STORAGE_BUCKET_ID', value: 'documents', secret: false },
  { key: 'DATABASE_ID', value: 'estately-main', secret: false },
  { key: 'TEMPLATE_COLLECTION_ID', value: 'contract_templates', secret: false },
...
function postVar(v) {
  return new Promise((resolve, reject) => {
    ...
    const url = new URL(`${ENDPOINT}/functions/upload-template/vars`);
    const opts = {
...
(async () => {
  for (const v of vars) {
    try {
      ...
      console.log(v.key, '->', r.status, r.body);
    } catch (e) {
