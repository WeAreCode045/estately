...
const tarPath = `${outDir}/upload-template.tar.gz`;

console.log('Creating tarball:', tarPath);
const tar = spawnSync('tar', ['-C', 'appwrite-functions/upload-template', '-czf', tarPath, '.'], { stdio: 'inherit' });
if (tar.status !== 0) {
...
const createArgs = [
  ...
  '-d', JSON.stringify({ functionId: 'upload-template', name: 'Upload Template', runtime: 'node-16.0', entrypoint: 'index.js' })
];
...
const deployArgs = [
  '-s',
  '-X', 'POST',
  `${ENDPOINT}/functions/upload-template/deployments`,
  '-H', `X-Appwrite-Project: ${PROJECT}`,
  '-H', `X-Appwrite-Key: ${KEY}`,
  '-F', `code=@${tarPath}`,
  '-F', 'activate=true'
...
try { fs.writeFileSync(`${outDir}/upload-deploy.json`, deployOut); } catch (e) { /* ignore */ }
