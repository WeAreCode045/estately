...
export class GeminiService {
  ...
  async generateContractDraft(
    ...
    const prompt = `
      ...
      - Project Ref: ${project.$id.substring(0, 8).toUpperCase()}
  ...
  async getProjectInsights(project: Project) {
    const prompt = `
      ...
      Price: $${project.price?.toLocaleString() || 'Not set'}
      Handover Date: ${project.handover_date || 'Not set'}

      Note: Tasks and milestones are managed separately and should be analyzed through the project management system.

      Return as a concise summary focusing on timeline, pricing, and overall project status.
    `;
