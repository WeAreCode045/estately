...
export const profileService = {
    ...
    /**
     * @deprecated Use taskService.create() with assignee_id instead
     * This method uses legacy JSON array storage in profiles
     *
     * Migration: taskService.create({ ...taskData, assignee_id: profileId })
     */
    async assignTask(profileId: string, taskId: string, extra?: { status?: 'PENDING' | 'COMPLETED', completedAt?: string, projectId?: string, title?: string, description?: string, dueDate?: string }) {
        console.warn('profileService.assignTask is deprecated. Use taskService.create() instead.');
        let profile: any;
    ...
    /**
     * @deprecated Use taskService.updateStatus() instead
     * This method uses legacy JSON array storage in profiles
     *
     * Migration: taskService.updateStatus(taskId, 'completed' | 'todo')
     */
    async updateTaskStatus(profileId: string, taskId: string, status: 'PENDING' | 'COMPLETED') {
        console.warn('profileService.updateTaskStatus is deprecated. Use taskService.updateStatus() instead.');
        let profile: any;
        let actualProfileId = profileId;
