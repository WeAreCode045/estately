...
import { COLLECTIONS, DATABASE_ID, ID, client, databases, inviteService, profileService, projectFormsService, projectService, taskService, documentRecordService } from '../api/appwrite';
...
import type { Contract, ContractTemplate, FormDefinition, FormSubmission, Project, ProjectTask, Task, TaskTemplate, User, UserDocumentDefinition, DocumentRecord } from '../types';
...
const ProjectDetail: React.FC<ProjectDetailProps> = ({ projects, setProjects, contracts, setContracts, templates, user, allUsers, onRefresh, onSwitchUser }) => {
  ...
  useEffect(() => {
    ...
    if (id) {
      fetchProjectTasksAndDocs();
    }
  }, [id, isAdmin]);

  const fetchProjectTasksAndDocs = async () => {
    if (!id) return;
    try {
      const [tasks, docs] = await Promise.all([
        taskService.listByProject(id),
        documentRecordService.listByProject(id)
      ]);
      setProjectTasks(tasks);
      setProjectDocuments(docs);
    } catch (error) {
      console.error('Error fetching project tasks and docs:', error);
    }
  };
  ...
  const projectStatusData = React.useMemo(() => {
    if (!project) return { tasks: [], docs: [], vault: [] };
    
    // Map tasks from new relational model to legacy format for UI compatibility
    const tasks = projectTasks.map(task => {
      // Find the assignee user
      const assigneeProfile = allUsers.find(u => {
        // Task assignee_id is profile.$id, need to match with user.id via profile
        return u.id && u.$id === task.assignee_id;
      });
      
      // Determine role based on project participants
      let role: UserRole = UserRole.ADMIN;
      if (assigneeProfile?.id === project.sellerId) role = UserRole.SELLER;
      else if (assigneeProfile?.id === project.buyerId) role = UserRole.BUYER;
      ...
      return {
        ...
        userName: assigneeProfile?.name || 'Unknown',
        projectId: task.project_id,
        dueDate: task.due_date
      };
    });

    // Map documents to legacy vault format
    const vault = projectDocuments.map(doc => {
      const ownerProfile = allUsers.find(u => u.$id === doc.owner_id);
      let role: UserRole = UserRole.ADMIN;
      if (ownerProfile?.id === project.sellerId) role = UserRole.SELLER;
      else if (ownerProfile?.id === project.buyerId) role = UserRole.BUYER;
      ...
      return {
        ...
        userDocumentDefinitionId: doc.type, // Map type to definition for backwards compat
        role,
        user: ownerProfile
      };
    });

    // Build combinedDocs structure for required docs tracking
    const combinedDocs = requiredDocs.map(rd => {
    ...
    return { tasks, docs: combinedDocs, vault };
  }, [project, projectTasks, projectDocuments, allUsers, requiredDocs]);
  ...
  const handleRejectDoc = async (uid: string, taskId: string) => {
    const reason = window.prompt('Reason for rejection:');
    if (reason === null) return;
    try {
      await taskService.updateStatus(taskId, 'todo');
      await fetchProjectTasksAndDocs();
      onRefresh?.();
    } catch (error) {
