    ...
    <div className="space-y-8 animate-in fade-in duration-300">
        ...
        <div className="bg-white rounded-[32px] border border-slate-100 shadow-sm overflow-hidden">
          <div className="p-6 border-b border-slate-100 flex items-center justify-between">
            <h2 className="text-lg font-bold text-slate-900 flex items-center gap-2">
              <Home size={20} className="text-indigo-600" /> Property Information
            </h2>
          </div>
          <div className="p-6 grid grid-cols-1 lg:grid-cols-2 gap-6">
            {/* Location */}
            {propertyData.locationData && (
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-600">
                    {propertyData.locationData.street}{propertyData.locationData.streetNumber ? ` ${propertyData.locationData.streetNumber}` : ''}
                  </p>
                  <p className="text-sm text-slate-500">
                    {propertyData.locationData.postalCode} {propertyData.locationData.city}
                  </p>
                </div>
              </div>
            )}
            ...
            {propertyData.sizeData && (
              <div className="flex items-start gap-4">
                ...
                <div>
                  <h3 className="font-bold text-slate-900 mb-1">Size</h3>
                  {propertyData.sizeData.floorSize && (
                    <p className="text-sm text-slate-600">Floor Area: {propertyData.sizeData.floorSize} m²</p>
                  )}
                  {propertyData.sizeData.lotSize && (
                    <p className="text-sm text-slate-600">Lot: {propertyData.sizeData.lotSize} m²</p>
                  )}
                </div>
              </div>
            )}
      ...
      {isLoadingProperty && (
        <div className="bg-white rounded-[32px] border border-slate-100 shadow-sm p-12 text-center">
          <p className="text-slate-400">Loading property information...</p>
        </div>
      )}
